        ...
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" style={{ zIndex: 9999 }}>
          ...
          >
            <div className="position-absolute top-0 start-50 translate-middle-x w-100 h-100" style={{ zIndex: -1 }}>
              <div className={`w-100 h-100 opacity-10 ${cfg.glowClass} rounded-circle animate-pulse`} style={{ filter: 'blur(50px)' }} />
            </div>

            <div className={`mb-4 d-inline-block p-3 rounded-circle bg-opacity-10 ${cfg.iconClass.replace('text-', 'bg-')} ${cfg.iconClass}`}>
              <Icon size={64} />
            </div>

            <h2 className="brand-title h3 mb-2 fw-bold">{title}</h2>
            <p className={`mb-4 ${isDarkMode ? 'text-secondary font-light' : 'text-secondary'}`}>{message}</p>

            <button onClick={onClose} className="btn btn-purple w-100 py-3 rounded-pill fw-bold shadow-lg transition-all hover-scale">
              {buttonText}
            </button>
